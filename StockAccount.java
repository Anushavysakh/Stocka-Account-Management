...
public class StockAccount extends CompanyShares implements CustomerAccountData {
	...
	public double valueOf() // total value of account dollars
	{
		...
		System.out.println("\nThe total value of all the stocks in account = " + val_stock);
		return val_stock;
	}
	/*
	...
	public void buy(int amount, double value, String symbol) // add shares of stock to account
	{
		...
		if (indx == -1) {
			System.out.println("\nYou don't have any shares of this company at present.");
			super.getStock_symbol().add(symbol);
		}
		...
		System.out.println("\nThe shares have been added successfully");
	}
	...
	public void printreport() {
		...
		for (int i = 0; i < super.getStock_symbol().size(); i++) {
			System.out.println(super.getStock_symbol().get(i) + "------------" + super.getStock_nos().get(i)
	...
	public static void main(String[] args) {
		...
		sa.stockAccount("Amazon", 32, 5000);
		sa.valueOf();
		sa.buy(5, 100, "Amazon");
		sa.valueOf();
		sa.sell(12, 600, "Amazon");
		sa.valueOf();
		sa.stockAccount("Reliance", 20, 2600);
		sa.valueOf();
		sa.buy(5, 99, "Reliance");
		sa.valueOf();
		sa.sell(9, 400, "Reliance");
		sa.valueOf();
		sa.printreport();
	}
}